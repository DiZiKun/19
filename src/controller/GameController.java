        ...
        OperatePanel.opButtons.get(1).addActionListener(e -> {
            if (ForbiddenIslandGame.getActionCount() < 3) { // Check action count limit
                if (ElementEngine.getBoard().isCanMove()) { // Check if movement is allowed
                    ElementEngine.moveTo();  // Execute move operation
                    LogUtil.console("Move To " + Arrays.toString(Map.coordinatesMatcher.get(ElementEngine.getAdventurers()[ForbiddenIslandGame.getRoundNum()].getPos())));
